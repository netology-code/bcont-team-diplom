...
Остальные участники выполняюи следующие действия:
* Забирают эти данные 
...
* Забирают к себе динамические разделы других участников.
...
2. Удалите лишние и создайте необходимые инфоблоки Меню, Отзывы, Мероприятия
3. Создайте структуру сайта, удалите лишние разделы и страницы
4. Интегрируйте шаблоны сайта.
...
* На главной странице должно быть по одному простому компоненту для вывода элементов из каждого раздела (см. файл index.html в вёрстке). Компоненты могут быть одинаковые (рекомендуем bitrix.news.line или bitrix.news.list)
* Для создания разделов используйте комплексные компоненты bitrix.news
* Настройте компоненты на работу в режиме ЧПУ, аналогично настройкам инфоблоков в административном разделе
* У всех используемых компонентов должно быть настроено кеширование
...
Вёрстка находится в материалах к Курсовому проекту.
...
|3 этап|4 дня|Разработка разделов|
|4 этап|2 дня|Взаимопроверка, устранение ошибок|
...
|Повторная сдача проекта* (при необходимости)|2 дня|Отправка, обратная связь от проверяющего преподавателя|
...
|Создает раздел|“Меню"|"Отзывы"|“Мероприятия”|
|Проверяет и исправляет ошибки|"Меню", “Отзывы”|“Мероприятия", “Меню”|"Отзывы", “Мероприятия”|

Отведите три ветки - `catalog`,`reviews`, `events` для проведения код ревью.


## Правила сдачи проекта

- Все ветки слиты в `main`
- Структура сайта соответствует заданию
- Нет лишних объектов, оставшихся от демо-установки
- Созданы необходимые инфоблоки
- Разработаны разделы  “Меню”, "Отзывы", “Мероприятия”
- На главной странице размещены 3 компонента с элементами из разделов
- На сайте включено Автокеширование
- Проверьте, что при переходе по ссылкам в меню и на страницах сайта нет 404 ошибок
